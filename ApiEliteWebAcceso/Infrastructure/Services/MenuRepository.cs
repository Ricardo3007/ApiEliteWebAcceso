...
namespace ApiEliteWebAcceso.Infrastructure.Services
{
    public class MenuRepository : IMenuRepository
    {
        ...
        public async Task<List<MenuNodeDto>> GetMenuPermiso(List<int> idEmpresas, int? idRol = null)
        {
            string queryMenu = @"
                    SELECT DISTINCT 
                    ...
                    WHERE ae.PK_EMPRESA_C IN @idEmpresas
                    AND ame.ESTADO_C = 'A' 
                    AND ape.ESTADO_C = 'A'";

            var menuItems = (await _dbConnection.QueryAsync<MenuNodeDto>(
                                queryMenu,
                                new { IdEmpresas = idEmpresas }
                            )).ToList();
        ...
        public async Task<List<MenuNodeDto>> GetMenuUsuarioEmpresa(int idEmpresa, int? idRol = null)
        {
            string queryMenu = @"
                    SELECT DISTINCT 
                        ame.PK_OPCION_MENU_C AS Id,
                    ...
                    INNER JOIN ACC_PERMISO_EMPRESA ape ON ape.FK_APLICATIVO_C = ame.FK_APLICATIVO_C
                    INNER JOIN ACC_EMPRESA ae ON ae.PK_EMPRESA_C = ape.FK_EMPRESA_C
                    WHERE ae.PK_EMPRESA_C = @IdEmpresa
                    AND ame.ESTADO_C = 'A' 
                    AND ape.ESTADO_C = 'A'";

            var menuItems = (await _dbConnection.QueryAsync<MenuNodeDto>(queryMenu, new { IdEmpresa = idEmpresa })).ToList();
            HashSet<int> roleOptions = new();
        ...
        public async Task<List<MenuNodeDto>> GetMenuPermisoGrupoEmpresa(int idGrupoEmpresa, int? idRol = null)
        {
            string queryMenu = @"
                    SELECT DISTINCT 
                        ame.PK_OPCION_MENU_C AS Id,
                    ...
                    INNER JOIN ACC_PERMISO_EMPRESA ape ON ape.FK_APLICATIVO_C = ame.FK_APLICATIVO_C
                    INNER JOIN ACC_EMPRESA ae ON ae.PK_EMPRESA_C = ape.FK_EMPRESA_C
                    WHERE ae.PK_EMPRESA_C = @IdGrupoEmpresa
                    AND ame.ESTADO_C = 'A' 
                    AND ape.ESTADO_C = 'A'";
